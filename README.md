...
The optimized Random Forest model was then evaluated again after removing the id feature.

Removing the id feature was important because it is not a medical characteristic and should not influence the prediction.
...
After removing the id feature, the models were evaluated on the test dataset.
...
The optimized Random Forest achieved the highest ROC-AUC among the two Random Forest configurations evaluated after removing the ID feature.

The ROC-AUC was approximately:

93.19%

The results demonstrate that the model has a good ability to distinguish between the two classes on the test dataset.
...
The id feature was explicitly removed before model training to prevent it from influencing feature importance.
...
- risk_level: The application risk category.
